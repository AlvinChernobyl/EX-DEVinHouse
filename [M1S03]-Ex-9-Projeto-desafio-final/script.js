...
var convidado4 = { nome: "Savio", sobrenome: "Silva", setor: "Pista", idade: 18 };
...
listaCamarote.forEach(element => {
    camarote.innerHTML += `<li>${element.nome} ${element.sobrenome} ${element.idade>17? '🍹':''}</li>`
});

listaArquibancada.forEach(element => {
    arquibancada.innerHTML += `<li>${element.nome} ${element.sobrenome} ${element.idade>17? '🍹':''}</li>`
});

listaPista.forEach(element => {

    pista.innerHTML += `<li>${element.nome} ${element.sobrenome} ${element.idade>17? '🍹':''}</li>`
});